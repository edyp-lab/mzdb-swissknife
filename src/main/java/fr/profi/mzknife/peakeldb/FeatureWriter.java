...
public class FeatureWriter {
  ...
  public static final String[] FT_COLUMNS = new String[]{"ion.id", "ion.mz", "ion.charge", "ion.rt", "ft.totalMatchedPeakelsCount", "ft.reliableMatchedPeakelsCount", "ft.reliableMatchedPeakelIds", "ft.intensity", "ft.area", "ft.elution_time", "ft.mz", "peakels.count", "peakel.ids", "ft.base_peakel_idx", "ft.isReliable", "cluster.peakels.ids", "ft.faims.cv", "ft.rawfile"};
  ...
  private static void writeMatchedFeature(PutativeFeatureWrapper putativeFt, StringBuilder strBuilder, CharSequence delimiter) {
    ...
    strBuilder.append(putativeFt.elutionTime()).append(delimiter);
     ...
     strBuilder.append(putativeFt.getTotalMatchedPeakelsCount()).append(delimiter);
     strBuilder.append(putativeFt.getReliableMatchedPeakelsCount()).append(delimiter);
     if (putativeFt.getReliableMatchedPeakelIds() != null) {
        strBuilder.append(putativeFt.getReliableMatchedPeakelIds().stream().map(i -> Integer.toString(i)).collect(Collectors.joining(listSeparator, "{", "}"))).append(delimiter);
     } else {
     ...
     if (putativeFt.isMatched()) {
      ...
      // no experimental feature found, output empty columns except for columns [ID, MZ, CHARGE, RT, TotalMatchedPeakelsCount, ReliableMatchedPeakelsCount, getReliableMatchedPeakelIds, CV, RAW]
      strBuilder.append(String.valueOf(delimiter).repeat(FT_COLUMNS.length - 9));
    }
