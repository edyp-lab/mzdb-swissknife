...
public class PeakelsDbFinder {
   ...
   private RTree<Integer, Point> buildRTree(Iterator<Peakel> peakelsIt) {
      ...
      LOG.info("rTree completed with {} entries", rTree.size());
      return rTree;
   }
   ...
   public List<PutativeFeatureWrapper> matchIdentifiedPsms(List<PutativeFeatureWrapper> psms, MzDbReader reader, Float mzTolPPM, Boolean groupPSMs) throws Exception {
      ...
      while (peakelsIt.hasNext()) {
         ...
         final int firstCycle = reader.getSpectrumHeaderForTime(peakel.getFirstElutionTime(), 1).getCycle() - 1;
...
//            matchedPsms.add(psm.id());
   ...
   private Feature buildFeature(Peakel peakel, PutativeFeature putativeFt, Float mzTolPPM) {
      ...
      return new FeatureAdapter(Feature.generateNewId(), peakel.getMz(), putativeFt.charge(), indexedPeakels, false, new long[0]);
   }
   ...
   public List<PutativeFeatureWrapper> matchPutativeFeatures(List<PutativeFeatureWrapper> putativeFts, File assignedPeakelsFile, Float mzTolPPM) throws IOException {
      ...
      for (PutativeFeatureWrapper putativeFt : putativeFts) {
         ...
         try {
            ...
            final Option<ArrayBuffer<Tuple2<Peakel, Object>>> matchingPeakels = PeakelDbHelper$.MODULE$.selectMatchingPeakels(
            ...
            if (matchingPeakels.isDefined()) {

               // TODO :count matching peakels and insert the information into the putative feature
               final List<Tuple2<Peakel, Object>> list = JavaConverters.bufferAsJavaList(matchingPeakels.get());
               int totalPeakelsCount = list.size();
               List<Peakel> reliablePeakels = list.stream().filter(t -> (Boolean)t._2).map(t -> t._1).toList();

               putativeFt.setTotalMatchedPeakelsCount(totalPeakelsCount);
               putativeFt.setReliableMatchedPeakelsCount((int)reliablePeakels.size());
               putativeFt.setReliableMatchedPeakelIds(reliablePeakels.stream().map(p -> p.getId()).toList());

               final Option<Tuple2<Peakel, Object>> matchingPeakel = PeakelDbHelper$.MODULE$.selectMatchingPeakel(putativeFt, matchingPeakels.get(), metrics);
   ...
   private static scala.collection.immutable.Set<Object> getAssignedPeakelIds(File assignedPeakelsFile) throws IOException {
      ...
      try {
         if (assignedPeakelsFile != null) {
            ...
            LOG.info("Assigned peakels list size = {}", assignedPeakelIds != null ? assignedPeakelIds.size() : 0);
